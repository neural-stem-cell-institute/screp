...
head(enrich_results$Enriched_lists$'0'$GOBP$data)[,2:5]

head(enrich_results$Enriched_lists$'3'$REACTOME$data)[,2:5]

head(enrich_results$Enriched_df$'0'$Pathways)[,2:5]

head(enrich_results$Enriched_df$'2'$GOBP)[,2:5]

## ---- warning=FALSE-----------------------------------------------------------
## Make the vector for the GO terms variable

goterms<-AnnotationDbi::Term(GO.db::GOTERM)
goterms<-prepGOTERMS(goterms)
...
go_vis_res2<-GO_viz_choose(go_vis_res1,chosen_cats=newcats,markers_df=markers, goterms=goterms)
...
## -----------------------------------------------------------------------------
react_vis1<-reactome_visualization(RP_ready,mRPR,RP=RP)
...
z
react_vis2<-reactome_visualization(RP_ready,mRPR,RP=RP,path_order=z)
